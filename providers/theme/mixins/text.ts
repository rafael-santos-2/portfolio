...
function getFontSize(fontSize: React.CSSProperties['fontSize']): number | undefined {
  return typeof fontSize === 'string' ? remToPx(fontSize) : fontSize;
}

function getLineHeight(lineHeight: React.CSSProperties['lineHeight'], fontSize?: number): number | undefined {
...
export function maxLine({ line, persistent }: TMaxLineProps): CSSObject {
  ...
  const responsiveStyles = breakpoints.reduce((acc, breakpoint) => {
    const fontSize = getFontSize(
      (persistent as TMediaFontSize)[defaultMuiTheme.breakpoints.up(breakpoint)]?.fontSize
    );
