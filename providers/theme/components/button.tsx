...
export type TButtonExtendVariant = {
  soft: true;
};
...
function style_colors(ownerState: ButtonProps, styles: (val: TPaletteColor) => CSSObject): CSSObject {
...
const MuiButtonBase: Components<Theme>['MuiButtonBase'] = {
   ...
   * STYLE
   *************************************** */
  styleOverrides: { root: ({ theme }) => ({ fontFamily: theme.typography.fontFamily , fontWeight:400 }) },
};
...
const softVariant: Record<string, ComponentsVariants<Theme>['MuiButton']> = {
  colors: COLORS.map((color) => ({
    ...
    style: ({ theme }) => ({
      ...
      '&:hover': { backgroundColor: varAlpha(theme.palette[color].mainChannel, 0.32) },
...
const MuiButton: Components<Theme>['MuiButton'] = {
  ...
  styleOverrides: {
    ...
    outlined: ({ theme, ownerState }) => {
      const styled = {
        colors: style_colors(ownerState, (color) => ({
          borderColor: varAlpha(theme.palette[color].mainChannel, 0.48),
        })),
        inheritColor: {
          ...(ownerState.color === 'inherit' &&
            !ownerState.disabled && {
